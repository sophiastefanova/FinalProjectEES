...
namespace MajorProjectEES
{
    public partial class LogIn : Form
    {
        ...
        {
            string sqlCon = @"Data Source=LAB109PC16\SQLEXPRESS; Initial Catalog=MajorProjectEES; Integrated Security=True;";
                ...
                try
                {
                    ...
                    if (CheckUsernameErrors() && CheckPasswordErrors())
                    {
                        ...
                        if (password.Text == password_2.Text)
                        {
                            ...
                            string Query1 = $"SELECT UserID, Date, Role FROM Users WHERE Username=@Username";
                            ...
                            using (SqlDataReader oReader = cmd.ExecuteReader())
                            {
                                if (oReader.Read())
                                {
                                    userID = Convert.ToInt32(oReader["UserID"]);
                                    ...
                                    if (count == 1)
                                    {
                                        ...
                                        {
                                            StudentView studentWindow = new StudentView(userID);
                                            studentWindow.Show();
                                            this.Hide();
